# PostgreSQL 오브젝트 생성 가이드 (자동차 정비예약 시스템)
...
> 본 가이드는 자동차 정비예약 시스템의 PostgreSQL 데이터베이스 오브젝트 설계 및 생성 시 일관성과 품질을 보장하기 위한 표준 규칙을 제공합니다.

## 예시
```sql
-- 테이블 생성 예시 (가이드 규칙 반영)
CREATE TABLE carfix.service_center (
...
-- 테이블 코멘트
COMMENT ON TABLE carfix.service_center IS '정비소 정보 테이블';

-- 컬럼 코멘트
COMMENT ON COLUMN carfix.service_center.service_center_id IS '정비소 ID';
COMMENT ON COLUMN carfix.service_center.business_number IS '사업자등록번호';
COMMENT ON COLUMN carfix.service_center.center_name IS '정비소명';
COMMENT ON COLUMN carfix.service_center.center_code IS '정비소 코드';
COMMENT ON COLUMN carfix.service_center.center_status_code IS '정비소 상태 코드 (코드그룹: CENTER_STATUS, 예시: ACTIVE, INACTIVE, SUSPENDED, CLOSED)';
COMMENT ON COLUMN carfix.service_center.address IS '주소';
COMMENT ON COLUMN carfix.service_center.phone_number IS '전화번호';
COMMENT ON COLUMN carfix.service_center.email IS '이메일';
COMMENT ON COLUMN carfix.service_center.operating_hours IS '운영시간';
COMMENT ON COLUMN carfix.service_center.description IS '설명';
COMMENT ON COLUMN carfix.service_center.created_at IS '생성일시';
COMMENT ON COLUMN carfix.service_center.updated_at IS '수정일시';
COMMENT ON COLUMN carfix.service_center.created_by IS '생성자';
COMMENT ON COLUMN carfix.service_center.updated_by IS '수정자';

-- 인덱스 생성 예시
CREATE INDEX service_center_ix1 ON carfix.service_center (center_status_code);
CREATE INDEX service_center_ix2 ON carfix.service_center (center_name);
```
