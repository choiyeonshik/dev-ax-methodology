<!--
Purpose: 자동차 정비예약 시스템의 DB 사용자/역할/권한 관리 가이드
Scope: 자동차 정비예약 시스템의 DB 사용자 및 권한 관리 시 적용
-->

# DB 역할 및 사용자 관리 가이드

이 문서는 자동차 정비예약 시스템의 데이터베이스(DB) 역할, 사용자, 권한 관리 원칙과 실제 적용 예시를 안내합니다. 실제 SQL 스크립트는 [`create_roles_and_users.sql`](./create_roles_and_users.sql)을 참고하세요.
...
CREATE USER carfix_admin WITH PASSWORD 'Admin****' IN ROLE carfix_admin_role;
CREATE USER carfix_developer WITH PASSWORD 'Dev****' IN ROLE carfix_developer_role;
CREATE USER carfix_api WITH PASSWORD 'Api****' IN ROLE carfix_api_role;
CREATE USER carfix_readonly WITH PASSWORD 'Read****' IN ROLE carfix_readonly_role;
```
...
### 4.1 Admin Role (`carfix_admin_role`)
- **권한**: 모든 DDL 권한 (CREATE, ALTER, DROP, TRUNCATE)
- **용도**: 데이터베이스 스키마 및 테이블 구조 관리
- **접근 가능**: carfix 스키마의 모든 객체

### 4.2 Developer Role (`carfix_developer_role`)
- **권한**: DML 권한 (SELECT, INSERT, UPDATE, DELETE)
- **용도**: 개발 및 테스트 환경에서의 데이터 조작
- **접근 가능**: carfix 스키마의 모든 테이블

### 4.3 API Role (`carfix_api_role`)
...
### 4.4 Read-Only Role (`carfix_readonly_role`)
- **권한**: SELECT 권한만
- **용도**: 보고서 생성, 데이터 분석, 비즈니스 인텔리전스
- **접근 가능**: carfix 스키마의 모든 테이블 (읽기 전용)
...
| 2.0   | 2024-12-19 | 관리자   | 자동차 정비예약 시스템에 맞게 전체 내용 수정 | carfix 시스템 적용 | 