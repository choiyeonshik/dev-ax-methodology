...
## 개요
이 폴더는 자동차 정비예약 시스템의 PostgreSQL 데이터베이스 설계, 구축, 관리를 위한 가이드 문서들을 포함합니다.
...
### carfix (Car Center Management System)
- **목적**: 자동차 정비예약 시스템의 핵심 비즈니스 로직 관리
- **주요 도메인**:
  - **사용자 관리**: user, user_profile, user_address
  - **차량 관리**: vehicle, vehicle_brand, vehicle_model, vehicle_owner
  - **정비소 관리**: service_center, service_center_operating_hours, service_center_rating
  - **예약 관리**: reservation, reservation_detail, reservation_status_history
  - **견적 관리**: quote, quote_item, quote_status_history
  - **결제 관리**: payment, payment_method, payment_status_history
  - **리뷰 관리**: review, review_reply, review_rating
  - **알림 관리**: notification, notification_template, notification_history
...
> 이 가이드들은 자동차 정비예약 시스템의 데이터베이스 설계 및 관리 표준을 정의합니다. 새로운 개발이나 변경 시 반드시 이 가이드들을 참조하여 일관성을 유지해주세요. 